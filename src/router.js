...
// ─── Classificação de sim/não em respostas curtas ─────────────────
// Cobre variantes PT-BR comuns (formal, gírias, abreviações, áudio transcrito).
// Estratégia: match exato → prefixo natural → match interno (mensagens curtas).
// Quando ambos batem (raro), prevalece NÃO — bot não age em sinal ambíguo.
const SIM_EXACT = new Set([
  's', 'sim', 'ok', 'okay', 'k', 'pode', 'claro', 'feito', 'manda',
  'manda ver', 'manda bala', 'blz', 'beleza', 'uhum', 'aham', 'ahã',
  'perfeito', 'exato', 'exatamente', 'isso', 'isso aí', 'isso ai',
  'correto', 'certo', 'confirma', 'confirmado', 'yes', 'y',
  'tá', 'ta', 'tá certo', 'ta certo', 'tudo certo', 'fechou', 'fechado',
  'mandou bem', 'positivo', 'sim sim', 'simbora'
]);
const NAO_EXACT = new Set([
  'n', 'não', 'nao', 'no', 'nope', 'nada', 'errado', 'errada',
  'outra', 'outro', 'não é', 'nao é', 'não foi', 'nao foi',
  'ainda não', 'ainda nao', 'negativo', 'nada disso', 'nem', 'sem chance'
]);
const SIM_PREFIX = /^(sim\b|isso\b|ok\b|okay\b|pode\b|claro\b|perfeito\b|exato\b|correto\b|certo\b|manda\b|feito\b|blz\b|beleza\b|confirma\b|fechou\b)/i;
const NAO_PREFIX = /^(não\b|nao\b|errad[oa]\b|outr[oa]\b|ainda não\b|ainda nao\b|nem\b|nada\b|negativo\b)/i;
const SIM_CONTAINS = /(^|\s)(sim|isso aí|isso ai|tá feito|ta feito|fechou|pode mandar)(\s|$|[!.?])/i;
const NAO_CONTAINS = /(^|\s)(não|nao|errad[oa]|nem fala|negativo)(\s|$|[!.?])/i;

function classifyYesNo(text) {
  const t = (text || '').toLowerCase().trim();
  if (!t) return { isSim: false, isNao: false };

  let isSim = SIM_EXACT.has(t);
  let isNao = NAO_EXACT.has(t);

  if (!isSim && SIM_PREFIX.test(t)) isSim = true;
  if (!isNao && NAO_PREFIX.test(t)) isNao = true;

  if (!isSim && !isNao && t.length <= 30) {
    if (SIM_CONTAINS.test(t)) isSim = true;
    if (NAO_CONTAINS.test(t)) isNao = true;
  }

  if (isSim && isNao) isSim = false;
  return { isSim, isNao };
}
...
async function process(body) {
  ...
  if (pendingConfirm) {
    ...
    // Robustez do sim/não: além de prefixo, aceita match exato, variantes PT-BR comuns,
    // e match no meio quando a mensagem é curta. Se ambos batem, prevalece NÃO (conservador).
